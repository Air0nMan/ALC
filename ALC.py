...
def diagonal(A): #devuelve una matriz que contiene solamente la diagonal de la matriz original A, y 0 en las demas posiciones
    res = np.zeros(A.shape) # creo una matriz de ceros con las mismas dimensiones que A
    for i in range (A.shape[0]): # recorro la diagonal y asigno los valores que corresponden
        res[i,i] = A[i,i]
    return res
...
def intercambiarFilas(A,i,j): # intercambia las filas i y j de la matriz A
    A=np.array(A)
    A[[i,j]] = A[[j,i]]  # intercambio las filas i y j
    return A


def sumarFilaMultiplo(A,i,j,s):
    A= np.array(A) # suma a la fila i la fila j multiplicada por el escalar s
    A[i]+= s*A[j]
    return A
...
def traza(A): #devuelve la traza de la matriz A
    res = 0
    for i in range (len(A)): # recorro la diagonal y sumo los valores
        res+= A[i][i]
    return res
...
def calcularAx (A, x): # calcula el producto Ax donde A es una matriz y x un vector
    ...
    for i in range(n): # recorro las filas de A
        suma = 0 # suma de la fila i
        for j in range(m): # recorro las columnas de A
            suma += A[i][j]*x[j] # multiplico y sumo
        res.append(suma) # agrego el resultado al vector res
    return np.array(res)
...
def esDiagonalmenteDominante(A): #verifica si la matriz A es diagonalmente dominante
    for i in range (len(A)): # recorro las filas de A
        suma = 0 # suma de los elementos no diagonales en la fila i
        diagonal = 0 # valor absoluto del elemento diagonal en la fila i
        for j in range (len(A[0])): # recorro las columnas de A
            if i == j:
                diagonal = abs(A[i][j])
            else:
                suma+= abs(A[i][j])
...
def producto_externo(x,y): # calcula el producto externo de dos vectores x e y
    A = np.zeros((len(x),len(y))) # creo una matriz de ceros de tamaño len(x) x len(y)

    for i in range(len(x)): # recorro las filas
        for j in range(len(y)):# recorro las columnas
            A[i][j] = x[i] * y[j]# asigno el valor del producto externo
    return A
...
def multiplicar_matrices(A, B): # calcula el producto de dos matrices A y B
    ...
    n1, p = B.shape# dimensiones de B
    if n != n1:# verifico que las matrices se puedan multiplicar
      return None
    C = np.zeros((m, p))# creo una matriz de ceros de tamaño m x p
...
def traspuesta(A): #devuelve la traspuesta de la matriz A
    ...
    for i in range(m): # recorro las filas de la traspuesta
        for j in range(n): # recorro las columnas de la traspuesta
...
def esSimetrica(A,atol=0): #verifica si la matriz A es simetrica con una tolerancia atol
  ...
  if filas!=columnas: # una matriz no cuadrada no puede ser simetrica
   return False
  for i in range(filas):# recorro las filas
   for j in range(filas):# recorro las columnas
...
def error_relativo(x,y): # calcula el error relativo entre dos valores x e y
  ...
  if x==0:# evito division por cero
    return np.inf
  else:
      return np.abs(x-y)/np.abs(x) # calculo el error relativo
...
def matricesiguales (A,B,tol=0): #verifica si dos matrices A y B son iguales con una tolerancia tol
  if A.shape!= B.shape: # Si las matrices tienen dimensiones distintas, no son iguales
    return False
  for i in range (A.shape[0]):# recorro las filas
      for j in range (A.shape[1]):# recorro las columnas
...
def rota(theta): #devuelve la matriz de rotacion de angulo theta
  cos_t=np.cos(theta)# calculo el coseno y seno de theta
  sen_t=np.sin(theta)
...
def norma(x,p): # calcula la norma p de un vector x
    if(p == "inf"): #se rompe en lista vacia
        max = abs(x[0])# inicio el maximo
        for i in x:
           if(max < abs(i)): # actualizo el maximo
               max = abs(i)
        return max
...
def normaExacta(a,p=[1,"inf"]): 
    if p == 2: # la norma 2 no se calcula exactamente es muy cara
        return None
    ...
    for i in range(0,cantFilas): 
        ...
        for j in range(0, CantColumnas):
            sum += abs(a[i][j])# suma de los valores absolutos de la fila i 
        if sum > resinf: # actualizo el maximo (definicion de norma infinito)
            resinf = sum
    ...
    for j in range(0,CantColumnas):
        sum = 0
        for i in range(0, cantFilas):
            sum += abs(a[i][j]) # suma de los valores absolutos de la columna j
        if sum > res1: # actualizo el maximo (definicion de norma 1)
            res1 = sum
    if p == 1:
      return res1
    elif p == "inf":
...
def metpot2k(A, tol=1e-12, K=1000): # calcula el autovector y autovalor dominante de la matriz A usando el metodo de potencias
    x = len(A[0])
    v = np.random.randn(x)
    v = v / norma(v, 2)
...
def diagRH(A, tol=1e-15, K=100, n_total=None): # diagonaliza matriz usando Householder recursivo y método de potencias
    """
...
def transiciones_al_azar_continuas(n): # genera una matriz de transiciones aleatoria continua de tamaño n x n
    M = np.random.rand(n, n)
    suma_columnas = M.sum(axis=0)
    M = M / suma_columnas
    return M

def transiciones_al_azar_uniformes(n, tol): 
    M = np.random.rand(n, n)
    M[M < tol] = 1.0
    M[M >= tol] = 0.0
...
def svd_reducida(A, k="max", tol=1e-15): # calcula la SVD reducida de la matriz A
    ...
    if m < n: # caso m < n, usamos la propiedad SVD(A) = (U, S, V) => SVD(A^T) = (V, S, U)
...
def pinvEcuacionesNormales(X,L, Y): # calcula la pseudoinversa de X usando cholesky
    ranX = rango(X)
    n,p = X.shape
    Lt = traspuesta(L)
    Xt = traspuesta(X)
...
def pinvHouseHolder(Q, R, Y): # calcula la pseudoinversa de X usando Householder
    n,p=R.shape
    Q = Q[:,:p]
    R = R[:p,:]
    Qt = traspuesta(Q)
...
def esPseudoInversa(X, pX, tol=1e-8): #verifica si pX es la pseudoinversa de X con una tolerancia
